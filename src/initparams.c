...
*  A new parameter rank1 is introduced.
*  More information about howto use this parameter
     ...
     int *pprintlevel;
{
  ...
  if (paramfile == NULL)
  ...
  else
    {
      ...
      fscanf(paramfile,"%*[^=]%*c%d",&(params->perturbobj));
      fscanf(paramfile,"%*[^=]%*c%d",&(params->fastmode));
      fscanf(paramfile,"%*[^=]%*c%d",&(params->rank1));
      fclose(paramfile);
    };
  ...
  if (*pprintlevel >= 3)
    {
      ...
      printf("params->perturbobj is %d \n",params->perturbobj);
      printf("params->fastmode is %d \n",params->fastmode);
      printf("params->rank1 is %d \n",params->rank1);
    };

...
/*
 * Modified with additional components needed for distributed parallel 
 * computations.
 * 
     ...
     struct scalapackpar scapack; 
{
  ...
  for (blk=1; blk<=A.nblocks; blk++)
	...
	case DIAG:
	  ...
	  break;
	case MATRIX:
	  ret=chol_blk(A.blocks[blk].blocksize,A.blocks[blk].blocksize,A.blocks[blk].data.mat,scapack);
	  break;
	default:

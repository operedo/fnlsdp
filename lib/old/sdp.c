...
*  nonzero value and Cholesky factorization is done in 
*  a distributed way.
*  
...
int sdp(n,k,C,a,constant_offset,constraints,byblocks,fill,X,y,Z,cholxinv,
	...
	Zi,O,O_d,RHS_d,descOd,mind_tmp,mind_gen,rhs,dZ,dX,dy,dy1,Fp,nfree,freevarblocks,
        freevarind, printlevel,parameters,scapack)
     int n;
     int k;
...
#define BASIZE 100
  ...
  /*  additional stuff for parallel use */

  int izero=0,ione=1,gi,gj;
  double diag_sqr;
  int pi,pj,li,lj,xi,xj;
  info=0;
  ...
  *dobj=calc_dobj(k,a,y,constant_offset);
  ...
  relpinfeas=pinfeas(k,constraints,X,a,workvec1,&parameters.rank1);
  bestrelpinfeas=relpinfeas;
  reldinfeas=dinfeas(k,C,constraints,y,Z,work2,&parameters.rank1);
  ...
  while ((relgap > parameters.objtol) || (relpinfeas > parameters.axtol) || (reldinfeas > parameters.atytol))
	 {
	   ...
	   if ((isdfeasprob==1) && (reldinfeas < parameters.atytol))
		 ...
		 printf("Got dual feasibility, so stopping.\n");
	       ...
	       make_i(work1);
	       zero_mat(work2);
	       op_a(k,constraints,work1,workvec1,&parameters.rank1);
	    ...
	    */
	   op_at(k,y,constraints,work1,&parameters.rank1);
	   ...
	   op_a(k,constraints,X,workvec1,&parameters.rank1);
	   ...
	   /*
	     Now, compute the system matrix O_d and its copy O.  
	   */
  
	   op_o(k,constraints,byblocks,Zi,X,O,O_d,descOd,work1,work2,work3,scapack,&parameters.rank1);
        
	   /*
	    * Print out the actual density of Z and X.
	    */
	   if ((iter==5) && (printlevel >= 3)&&(scapack.id==0))
	     {
        ...
        /*
          Here we separate the diagonal elements of O_d and find norm(diag(O_d))
          and min(diag(O_d)).
         */
         for (i=0; i<=k; i++){
             diagO[i]=0.0;
         ...
         mind_tmp[scapack.id]=1.0e30;
         diagnrm=0.0; diag_sqr=0.0;
         for (j=1;j<=k;j++){

              pi = p_of_i(j,scapack.nb,scapack.nprow);
              pj = p_of_i(j,scapack.nb,scapack.npcol);
               
             if (pi==scapack.myrow && pj == scapack.mycol){

                 xi = x_of_i(j,scapack.nb,scapack.nprow);
                 xj = x_of_i(j,scapack.nb,scapack.npcol);

                 li = l_of_i(j,scapack.nb,scapack.nprow);
                 lj = l_of_i(j,scapack.nb,scapack.npcol);

                ent = O_d[ijtok((li*scapack.nb+xi),(lj*scapack.nb+xj),scapack.lr)];
                workvec1[j]=ent;
                 i = (ijtok((li*scapack.nb+xi),(lj*scapack.nb+xj),scapack.lr));
                if (ent < mind_tmp[scapack.id] )
                    mind_tmp[scapack.id] =ent;
                diag_sqr = diag_sqr + ent*ent;
              }
          }


         MPI_Barrier(MPI_COMM_WORLD);
         MPI_Reduce(&diag_sqr,&diagnrm,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
         
         if (scapack.myrow==0 && scapack.mycol==0)
             diagnrm=sqrt(diagnrm);
         MPI_Bcast(&diagnrm,1,MPI_DOUBLE,0,MPI_COMM_WORLD);

        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Reduce(mind_tmp,mind_gen,scapack.np,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
        if (scapack.myrow==0 && scapack.mycol==0){
            mindiag = mind_gen[0];
            for (i=1;i<scapack.np;i++)
               if (mind_gen[i]<mindiag)
                   mindiag = mind_gen[i];

         }
            MPI_Bcast(&mindiag,1,MPI_DOUBLE,0,MPI_COMM_WORLD);

        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Reduce(workvec1,diagO,k+1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
        MPI_Bcast(diagO,k+1,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
RETRYFACTOR:
	   ...
	   if ((printlevel >= 3)&&(scapack.id==0))
         ...
         for (gj=1;gj<=k;gj++){
              ...
              workvec8[gj]=1.0/sqrt(diagO[gj]);

              if (workvec8[gj] > 1.0e30)
                    workvec8[gj]=1.0e30;

               if (scapack.myrow==indxg2p_(&gj,&scapack.nb,&scapack.myrow,&izero,&scapack.nprow) &&
                 scapack.mycol==indxg2p_(&gj,&scapack.nb,&scapack.mycol,&izero,&scapack.npcol)){

                  i=indxg2l_(&gj,&scapack.nb,&scapack.myrow,&izero,&scapack.nprow);
                  j=indxg2l_(&gj,&scapack.nb,&scapack.mycol,&izero,&scapack.npcol);

                  O_d[ijtok(i,j,scapack.lr)] += diagadd;
               }
	    ...
	    */
        ...
        for (gj=1; gj<=k; gj++)
           for (gi=1; gi<=gj; gi++){
              ...
              if (scapack.myrow==indxg2p_(&gi,&scapack.nb,&scapack.myrow,&izero,&scapack.nprow) &&
                 scapack.mycol==indxg2p_(&gj,&scapack.nb,&scapack.mycol,&izero,&scapack.npcol)){

                  i=indxg2l_(&gi,&scapack.nb,&scapack.myrow,&izero,&scapack.nprow);
                  j=indxg2l_(&gj,&scapack.nb,&scapack.mycol,&izero,&scapack.npcol);

                  O_d[ijtok(i,j,scapack.lr)] = O_d[ijtok(i,j,scapack.lr)]*(workvec8[gi]*workvec8[gj]);
               }
          }
    
           
	   /*
	     Next, compute the Cholesky factorization of the distributed system matrix.
           */  
...
#ifdef NOUNDERLAPACK
  #ifdef CAPSLAPACK
          PDPOTRF("U",&m,O_d,&ione,&ione,descOd,&info );
  #else
          pdpotrf("U",&m,O_d,&ione,&ione,descOd,&info );
  #endif
#else
  #ifdef CAPSLAPACK
          PDPOTRF_("U",&m,O_d,&ione,&ione,descOd,&info );
  #else
          pdpotrf_("U",&m,O_d,&ione,&ione,descOd,&info );
  #endif
#endif
	   ...
	   if (info != 0) 
	       ...
	       if (retries < 10)
		 {
		   ...
		   diagfact=diagfact*10.0;
                   ...
                   MPI_Barrier(MPI_COMM_WORLD);
                   pdlacpy_("All", &ldam, &ldam, O, &ione, &ione, descOd, O_d, &ione, &ione, descOd);
	   ...
	   */
	   ...
	   if ((bestmeas > 1.0e3) && (parameters.perturbobj==1))
	     {
	       if ((printlevel >= 3)&&(scapack.id==0))
		 printf("Perturbing C.\n");
	       make_i(work3);
	   ...
	   op_a(k,constraints,dX,workvec1,&parameters.rank1);
	   ...
	   */
	   ...
	   info=solvesys(k,ldam,descOd,O_d,RHS_d,workvec1,scapack);
	   for (i=1; i<=k; i++)
	     workvec1[i]=workvec1[i]*workvec8[i];
	   ...
	   if ((iter>1) && (relerr2 > parameters.axtol) && 
	     ...
	     {
	       op_at(k,workvec1,constraints,work1,&parameters.rank1);
	       mat_multspa(1.0,0.0,work1,X,dX,fill);
	       mat_multspc(1.0,0.0,Zi,dX,work1,fill);
	       op_a(k,constraints,work1,workvec2,&parameters.rank1);
	       for (i=1; i<=k; i++)
		      ...
		      && (besterr > 1.0e-14))
		 {
		   ...
		   info=solvesys(k,ldam,descOd,O_d,RHS_d,workvec3,scapack);
	   ...
	   */
	   ...
	   op_at(k,dy,constraints,dZ,&parameters.rank1);
	   ...
	   */
		...
		*/
	       ...
	       else
		 {
	       ...
	       affreldinfeas=dinfeas(k,C,constraints,workvec1,work2,work3,&parameters.rank1);
	       affrelpinfeas=pinfeas(k,constraints,work1,a,workvec1,&parameters.rank1);
	       ...
	       if (bestmeas < 1.0)
		 {
		   ...
		   iter=iter+1;
		   if (printlevel >= 1 && (scapack.myrow==0 && scapack.mycol == 0))
	   ...
	   */
	   ...
	   op_a(k,constraints,work1,Fp,&parameters.rank1);
	   for (i=1; i<=k; i++)
	     Fp[i]=a[i]-Fp[i];
	   ...
	   */
	   ...
	   op_a(k,constraints,work2,rhs,&parameters.rank1);
	   ...
	   */
	   ...
	   if ((iter>1) && (relerr2 > 0.01*parameters.axtol) &&
	       (parameters.fastmode==0))
	     {
	       ...
	       op_a(k,constraints,work3,workvec2,&parameters.rank1);
	       for (i=1; i<=k; i++)
		 workvec2[i]=rhs[i]-workvec2[i];
	   ...
	   */
	   ...
	   op_at(k,dy1,constraints,work3,&parameters.rank1);
	   ...
	   if ((printlevel >= 3)&&(scapack.id==0))
	     {
	       ...
	       if ((relerr1 < relerr2)&&(scapack.id==0))
	   ...
	   */
	   ...
	   newrelpinfeas=pinfeas(k,constraints,work1,a,workvec1,&parameters.rank1);
	   ...
	   newreldinfeas=dinfeas(k,C,constraints,workvec1,work1,work2,&parameters.rank1);
	   ...
	   */
	   ...
	   if (relpinfeas > 1.0)
	       ...
	       for (i=1; i<=9; i++)
		 {
		   ...
		   if (pinfeas(k,constraints,work1,a,workvec1,&parameters.rank1) < newrelpinfeas)
		     {
  ...
  if ((gap < 0.0) && (parameters.usexzgap==0) && (parameters.tweakgap==1)
      && (ispfeasprob==0) && (isdfeasprob==0))
    {
      tweakgap(n,k,a,constraints,gap,Z,dZ,y,dy,work1,work2,work3,dX,
	       workvec1,workvec2,workvec3,workvec4,printlevel,&parameters.rank1,
		scapack.id);
    };
  ...
  *dobj=calc_dobj(k,a,y,constant_offset);
  ...
  relpinfeas=pinfeas(k,constraints,X,a,workvec1,&parameters.rank1);
  reldinfeas=dinfeas(k,C,constraints,y,Z,work1,&parameters.rank1);

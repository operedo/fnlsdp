...
int read_prob(fname,pn,pk,pC,pa,pconstraints,printlevel,prank1,id)
     char *fname;
     int *pn;
     int *pk;
  ...
  for (i=1; i<=nblocks; i++)
    {   /*modified due to rank1 variable*/
      if ((pC->blocks[i].blockcategory != DIAG) &&
          (isdiag[i]==1) && (pC->blocks[i].blocksize > 1)&&(*prank1<i))
